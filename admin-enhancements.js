...
(() => {
  ...
  function franchiseSimilarity(first, second, firstPrepared = null, secondPrepared = null) {
    const a = firstPrepared?.compact || compactFranchise(first);
    const b = secondPrepared?.compact || compactFranchise(second);
    ...
    const aWords = firstPrepared?.words || franchiseWords(first);
    const bWords = secondPrepared?.words || franchiseWords(second);
  ...
  function buildSimilarFranchisePairs(openings) {
    ...
    const addCandidateGroup = (indexes, exact = false) => {
      for (let first = 0; first < indexes.length; first += 1) {
        for (let second = first + 1; second < indexes.length; second += 1) {
          ...
          const key = `${a}:${b}`;
          candidateCounts.set(key, exact ? 100 : (candidateCounts.get(key) || 0) + 1);
        }
      }
    };
    compactIndex.forEach(indexes => addCandidateGroup(indexes, true));
    tokenIndex.forEach(indexes => {
      if (indexes.length >= 2 && indexes.length <= 40) addCandidateGroup(indexes);
    });

    const pairs = [];
    const suggestedKeys = new Set();
    for (const [candidateKey, sharedTokenCount] of candidateCounts) {
      ...
      if (sharedTokenCount < 2 && Math.min(first.compact.length, second.compact.length) > 6) continue;
      const key = franchisePairKey(first.name, second.name);
      if (rejectedFranchisePairs.has(key) || suggestedKeys.has(key)) continue;
      const score = franchiseSimilarity(first.name, second.name, first, second);
