(() => {
  if (window.__OC_MANUAL_TOP_INSERT_READY__) return;
  ...
  async function loadCandidates(user, type) {
    ...
    const promise = (async () => {
      ...
      snapshots.forEach(snapshot => snapshot.docs.forEach(documentSnapshot => {
        ...
        const score = data.score !== undefined && data.score !== null && data.score !== '' ? Number(data.score)
          ...
          : null;
        if (Number.isFinite(score)) ratingRows.set(openingId, score);
      }));
  ...
  function itemRank(item) {
    const button = item?.querySelector('[data-action="set-rank"]');
  ...
  function imageHtml(entry, className = '') {
    const src = clean(entry.image || entry.fallbackImage);
    if (!src) return `<div class="${className} oc-manual-insert-noimage">${escapeHtml(entry.type || '')}</div>`;
    const fallback = clean(entry.fallbackImage);
    return `<img class="${className}" src="${escapeHtml(src)}" data-fallback="${escapeHtml(fallback)}" alt="" loading="lazy" decoding="async">`;
  }
  ...
  function candidateMatches(entry, queryText) {
    ...
    const values = [entry.title, ...(Array.isArray(entry.alternativeTitles) ? entry.alternativeTitles : [])].map(normalize);
  ...
  function candidateMeta(entry, rank) {
    ...
    const position = rank ? `сейчас №${rank}` : 'сейчас вне топ-100';
    return [date, position].filter(Boolean).join(' · ');
  }
  ...
  function dispatchControl(element) {
    if (!element) return;
    const eventName = element.matches('input[type="search"],input[type="text"],input[type="number"]') ? 'input' : 'change';
    element.dispatchEvent(new Event(eventName, { bubbles: true }));
  }

  async function restoreControls(snapshot) {
    PROFILE_FILTER_IDS.forEach(id => applyControlValue(document.getElementById(id), snapshot[id]));
    PROFILE_FILTER_IDS.forEach(id => dispatchControl(document.getElementById(id)));
    await sleep(80);
  }
  ...
  function exactPositionButton(id) {
    return actionButton('all-set-rank', id);
  }
  ...
  async function prepareCandidateInAllRatings(candidate, type) {
    ...
    if (arType) {
      arType.value = type;
      arType.dispatchEvent(new Event('change', { bubbles: true }));
    }
    const arScore = document.querySelector('#oc-ar-score');
    if (arScore) {
      arScore.value = '';
      arScore.dispatchEvent(new Event('change', { bubbles: true }));
    }
    const search = document.querySelector('#oc-p-search');
    if (search) {
      search.value = clean(candidate.title);
      search.dispatchEvent(new Event('input', { bubbles: true }));
    }
    await sleep(120);

    let rankButton = exactPositionButton(candidate.id);
    ...
    if (hiddenToggle && !hiddenToggle.disabled && /^Показать скрытые/i.test(clean(hiddenToggle.textContent))) {
      hiddenToggle.click();
      await sleep(120);
      const unhide = actionButton('all-unhide-manual', candidate.id);
      if (unhide) {
        unhide.click();
        await sleep(120);
      }
      rankButton = exactPositionButton(candidate.id);
    }
    return rankButton;
  }
  ...
  async function moveCandidate(candidate, type, targetPlace) {
    ...
    try {
      let rankButton = await prepareCandidateInAllRatings(candidate, type);
      let currentPlace = rankFromButton(rankButton);

      if (currentPlace && currentPlace <= targetPlace) {
        throw new Error(`Этот трек уже находится на ${currentPlace}-м месте — не ниже выбранной позиции.`);
      }
      ...
      if (!currentPlace) {
        ...
        if (currentPlace === targetPlace) return true;
        if (currentPlace < targetPlace) throw new Error(`Трек добавился на ${currentPlace}-е место, которое уже выше выбранной позиции.`);
      }
  ...
  function openPanel(zone, type, targetPlace) {
    ...
    panel.innerHTML = `
      <div class="oc-manual-insert-head">
        <div><strong>Добавить ${type} на ${targetPlace}-е место</strong><small>Доступны только треки, которые оценены владельцем профиля. Трек ниже выбранного места будет перенесён сюда.</small></div>
        <button type="button" class="oc-manual-insert-close" aria-label="Закрыть">×</button>
      </div>
      <div class="oc-manual-insert-search-wrap"><input class="oc-manual-insert-search" type="search" autocomplete="off" placeholder="Начните вводить название ${type}…"></div>
      <div class="oc-manual-insert-results"><div class="oc-manual-insert-loading">Загружаю оценённые треки…</div></div>
      <div class="oc-manual-insert-note">Изменение останется локальным до нажатия общей кнопки «Сохранить топ-100».</div>`;
    zone.append(panel);
    ...
    function renderPreview() {
      ...
      const rank = ranks.get(String(selected.id));
      const preview = document.createElement('div');
      preview.className = 'oc-manual-insert-preview';
      preview.innerHTML = `
        ${imageHtml(selected)}
        <div><div class="oc-manual-insert-preview-title">${escapeHtml(selected.title)}</div><div class="oc-manual-insert-preview-meta">${escapeHtml(candidateMeta(selected, rank))} · оценка ${escapeHtml(selected.manualInsertScore)}</div></div>
        <button type="button" class="oc-manual-insert-confirm">${rank ? `Переместить на ${targetPlace}-е место` : `Добавить на ${targetPlace}-е место`}</button>`;
    ...
    function renderResults() {
      ...
      const matches = candidates
        ...
        .sort((a, b) => {
          ...
          return b.manualInsertScore - a.manualInsertScore || clean(a.title).localeCompare(clean(b.title), 'ru');
        })
        .slice(0, 10);
      ...
      results.innerHTML = matches.map(entry => {
        const rank = ranks.get(String(entry.id));
        return `<button type="button" class="oc-manual-insert-result${selected && String(selected.id) === String(entry.id) ? ' selected' : ''}" data-id="${escapeHtml(entry.id)}">
          ${imageHtml(entry)}
          <span class="oc-manual-insert-result-main"><span class="oc-manual-insert-result-title">${escapeHtml(entry.title)}</span><span class="oc-manual-insert-result-meta">${escapeHtml(candidateMeta(entry, rank))}</span></span>
          <span class="oc-manual-insert-result-score">${escapeHtml(entry.manualInsertScore)}</span>
        </button>`;
      }).join('');
    ...
    loadCandidates(user, type).then(rows => {
      ...
      if (!panel.isConnected) return;
      results.innerHTML = `<div class="oc-manual-insert-error">${escapeHtml(error?.message || 'Не удалось загрузить оценки.')}</div>`;
    });
  }
  ...
  function makeZone(type, targetPlace) {
    ...
    zone.querySelector('button').addEventListener('click', () => openPanel(zone, type, targetPlace));
    return zone;
  }
  ...
  function mountList(container, type) {
    ...
    if (!items.length) {
      ...
      container.dataset.manualInsertSignature = '';
      return;
    }

    const signature = items.map(item => `${item.querySelector('[data-action="set-rank"]')?.dataset.id || ''}:${itemRank(item) || ''}`).join('|');
    if (container.dataset.manualInsertSignature === signature && container.querySelector(':scope > .oc-manual-insert-zone')) return;
    if (activeZone && container.contains(activeZone)) closePanel();
    container.querySelectorAll(':scope > .oc-manual-insert-zone').forEach(zone => zone.remove());
    container.dataset.manualInsertSignature = signature;
    ...
    if (lastRank && lastRank < 100 && !usedTargets.has(lastRank + 1)) {
      items[items.length - 1].after(makeZone(type, lastRank + 1));
    }
  }

  function autoExpandTopLists() {
    if (autoExpandBusy) return false;
    const buttons = [...document.querySelectorAll('[data-action="toggle-profile-top"]')]
      .filter(button => /^Показать весь топ/i.test(clean(button.textContent)));
    if (!buttons.length) return false;
    autoExpandBusy = true;
    buttons.forEach(button => button.click());
    window.setTimeout(() => { autoExpandBusy = false; scheduleMount(); }, 120);
    return true;
  }

  function mount() {
    const editing = manualEditingActive();
    ...
    if (!editing) {
      ...
      document.querySelectorAll('#oc-profile-op,#oc-profile-ed').forEach(container => { container.dataset.manualInsertSignature = ''; });
      return;
    }

    if (autoExpandTopLists()) return;
  ...
  function init() {
    ...
    new MutationObserver(records => {
      const meaningful = records.some(record => !record.target.closest?.('.oc-manual-insert-panel'));
      if (meaningful) scheduleMount();
  ...
  window.__OC_MANUAL_TOP_INSERT_READY__ = true;
